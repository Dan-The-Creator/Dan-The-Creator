<div align="center">

# Привет! Я Дэн 👋

### Prompt Engineer & AI Automation Developer

[![Telegram](https://img.shields.io/badge/Telegram-@dannisi-26A5E4?style=flat&logo=telegram&logoColor=white)](https://t.me/dannisi)
[![GitHub](https://img.shields.io/badge/GitHub-Dan--The--Creator-181717?style=flat&logo=github)](https://github.com/Dan-The-Creator)

</div>

---

## ⚡ Что я делаю

Автоматизирую бизнес-процессы с помощью AI:

- 🤖 **Telegram-боты** — транскрибация, генерация контента, консультанты
- 📊 **AI-аналитика** — автоанализ отзывов, контроль качества звонков
- 📱 **SMM-автоматизация** — генерация и публикация контента в соцсетях
- 💬 **AI-ассистенты** — техподдержка, консультанты на базе RAG

---

## 🛠 Технологии

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![Telegram](https://img.shields.io/badge/Telegram_Bot-26A5E4?style=flat&logo=telegram&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![Google Sheets](https://img.shields.io/badge/Google_Sheets-34A853?style=flat&logo=google-sheets&logoColor=white)
![Make](https://img.shields.io/badge/Make.com-6D00CC?style=flat&logo=make&logoColor=white)

---

## 📂 Избранные проекты

| Проект | Описание | Стек |
|--------|----------|------|
| [AI Samurai SMM](https://github.com/Dan-The-Creator/smm-expert) | Автопостинг в ВК с генерацией текста и изображений | Python, Flask, OpenAI, VK API |
| [VoiceScribe Bot](https://github.com/Dan-The-Creator/voicescribe-bot) | Транскрибация голосовых + выделение тезисов | Python, Whisper, GPT-4o |
| [RAG Jewelry Consultant](https://github.com/Dan-The-Creator/RAG-Jewelry-Consultant) | AI-консультант для ювелирного магазина | LangChain, FAISS, RAG |
| [Sales Call QC](https://github.com/Dan-The-Creator/Sales-Call-Quality-Control) | Контроль качества звонков отдела продаж | Make.com, OpenAI |

---

## 🎯 Сейчас

- 🏯 Строю AI-студию **AI Samurai**
- 🔍 Открыт к проектам по AI-автоматизации

---

<div align="center">

**Есть задача по автоматизации? Напишите в [Telegram](https://t.me/dannisi)**

</div>
